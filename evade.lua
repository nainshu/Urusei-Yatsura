...
local VoteTab = Window:CreateTab("投票")
local EspTab = Window:CreateTab("透视 (ESP)")
local ReviveTab = Window:CreateTab("复活")
local FakeTab = Window:CreateTab("外观")
...
VoteTab:CreateDropdown({
    Name = "选择地图",
    Options = {"地图 1", "地图 2", "地图 3", "地图 4"},
    CurrentOption = "地图 1",
    Flag = "MapSelection",
    Callback = function(Option)
        if Option == "地图 1" then
            selectedMapNumber = 1
        elseif Option == "地图 2" then
            selectedMapNumber = 2
        elseif Option == "地图 3" then
            selectedMapNumber = 3
        elseif Option == "地图 4" then
            selectedMapNumber = 4
        end
    end,
})
                ...
                if ticketsFolder then
                    for _, ticket in pairs(ticketsFolder:GetChildren()) do
                        if ticket and ticket.PrimaryPart and ticket.Name == "Visual" then
                            CreateEsp(ticket, Color3.new(0.9, 0.9, 0.3), "票券", ticket.PrimaryPart, -2)
                        end
                    end
                end
            end)
