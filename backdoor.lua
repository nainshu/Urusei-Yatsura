...
local remoteInfo = {
	...
	["srcFunc"] = nil,
	["redirection"] = {
		["__testver"] = false, -- 使用测试版可能不总是有效，请谨慎使用
		["initialized"] = false,
	}
}
local msgOutputs = {
	["mainTabText"] = "--[[\n\tjLn0n 的 beckdeer execeeter 已加载!\n\t正在使用 'github.com/jLn0n/executor-gui' 作为界面。\n\n\t文档: github.com/jLn0n/beckdeer-skenner/blob/main/README.MD\n\t社区服务器: https://discord.gg/jvb7XNzNPN \n--]]\n",

	["attached"] = "\n 已附加远程: %s\n 类型: %s\n 有效载荷: %s",
	["cacheLoaded"] = "已加载 [%s] 的地点缓存。",
	["printRemote"] = "\n 远程: %s | [%s]\n 类型: %s",

	["scanMsg1"] = "远程扫描已开始。",
	["scanMsg2"] = "如果 config.enableLogging 设置为 true，您可以在开发者控制台中看到正在扫描的远程。",
	["scanBenchmark"] = "扫描远程耗时 %.2f 秒。",

	["cacheFailed"] = "未能加载 [%s] 的后门缓存，它可能已过时。",
	["outdatedConfig"] = "配置文件已过时!\n建议更新配置以防止出错。",
	["configLoadFailed"] = "本地配置加载失败，可能已损坏。",
	["noBackdoorRemote"] = "这里找不到后门远程!",
	["remoteRedirectLoadFailed"] = "远程重定向加载失败，正在使用原始远程。",
}
local msgBoxParams = {
	["DiscordInvitePrompt"] = {
		Title = "Discord 邀请提示",
		TextContent = "您想加入我们的 Discord 社区服务器吗?",
		ButtonCount = 2,
	},
	["DiscordInviteCopied"] = {
		Title = "Discord 邀请提示",
		TextContent = "已复制 Discord 服务器邀请链接。",
		ButtonCount = 1,
		Button0Text = "确定"
	}
}
...
local function notSameRandNumber(min, max, ...)
	...
	task.defer(table.clear, numIndexes) -- 优化!?!!?
	return (
...
local applyRedirectedRemoteSecurity do
	...
	function applyRedirectedRemoteSecurity(source)
		...
		generatedArgs[verificationIdx] = true -- 在 `verificationIdx` 处设置一个布尔值
		generatedArgs[srcArgIdx] = base91.encodeString(XORSource(source, XORKey))
		generatedArgs[randIdx] = srcArgIdx -- 将 `srcArgIdx` 的值设置为 `randIdx`
		generatedArgs[nonceIdx] = `\127@{generateRandString(randIdx + nonceOffset)}` -- 生成一个长度为 `randIdx` 的 nonce

		for argIndex = 2, argsLenght do -- 插入随机的无意义内容
...
local function initRemoteRedirection()
	...
	execScript(`require({if remoteInfo.redirection.__testver then 11906414795 else 11906423264})("{nonce}", %userid%)`, true) -- 如果你想尝试新功能，将 'remoteInfo.redirection.__testver' 设置为 true
	waitUntil(5, function() return insertService:GetAttribute(nonce) end) -- 我们需要即兴创作，直到 :WaitForAttribute 被添加
...
-- main (主函数)
do -- config initialization (配置初始化)
...
do -- backdoor finding (后门查找)
	...
	if placeCacheData then
		...
		if not remoteInfo.foundBackdoor then -- 如果没有找到后门
